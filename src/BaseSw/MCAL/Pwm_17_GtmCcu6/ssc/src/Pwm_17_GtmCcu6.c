...
#if (PWM_17_GTMCCU6_SW_MINOR_VERSION != 40U)
...
/* MISRA2012_RULE_2_5_JUSTIFICATION: This macro is referenced only when certain
precompile configuration switch is enabled.*/
/* To store the MAX value for TOM and ATOM channels */
#define PWM_MAX_TIMER_CM_REG         ((uint32)0xFFFEU)
...
#if (PWM_17_GTMCCU6_GTM_TIMER_USED == STD_ON)
static uint8 Pwm_lCheckClkDiv(const Pwm_17_GtmCcu6_ChannelType ChannelNumber,
                             const uint8 CoreId,
                             const uint32 Period,
                             const uint32 Dutycycle);

static uint8 Pwm_lAtomCheckDuty(const Pwm_17_GtmCcu6_ChannelType ChannelNumber,
                                const uint8 CoreId,
                                const uint32 Period,
                                const uint32 Dutycycle,
                                const uint8 Err517Stat);
#if((PWM_17_GTMCCU6_SET_PERIOD_AND_DUTY_API == STD_ON) || \
    (PWM_17_GTMCCU6_SET_DUTY_CYCLE_API == STD_ON) ||      \
    (PWM_17_GTMCCU6_SET_OUTPUT_TO_IDLE_API == STD_ON))
static uint8 Pwm_lTomCheckDuty(const Pwm_17_GtmCcu6_ChannelType ChannelNumber,
                             const uint8 CoreId,
                             const uint32 Period,
                             const uint32 Dutycycle,
                             const uint32 Shift);
...
)
{
  ...
  /* [cover parentID={61D4A815-BCA1-4743-A7ED-1E589D5A7119}]
     If HW Errata GTM_AI.517 and GTM_AI.522 is applicable and workaround
     applied if applicable [/cover]*/
  (void)Pwm_lCheckClkDiv(ChannelNumber, CoreId,
                         PwmGtmTimerPtr->TimerChCM0Reg,
                         PwmGtmTimerPtr->TimerChCM1Reg);
...
)
{
  ...
  if(PwmGtmTimerType == MCU_GTM_TIMER_TOM)
  {
    ...
    Enable TOM Channel Output stat register [/cover] */
    Mcu_17_Gtm_TomChOutEnStatUpdate(TimerModuleNumber, TimerChannelNumber, \
                                    MCU_GTM_TIMER_OUT_ENABLE);
  }
  else
  {
    ...
    Enable ATOM Channel Output stat register [/cover] */
    Mcu_17_Gtm_AtomChOutEnStatUpdate(TimerModuleNumber, TimerChannelNumber, \
                                    MCU_GTM_TIMER_OUT_ENABLE);
  }
}
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(Dutycycle == 0U)
      {
        /* [cover parentID={399DE101-236E-4b68-852B-C891065CAE9E}]
         If HW Errata GTM_AI.522 is applicable and workaround applied
         if applicable [/cover]*/
        RegUpdated = Pwm_lTomCheckDuty(ChannelNumber, CoreId, Period, \
                                       Dutycycle, Shift);
        /* [cover parentID={5BD6200E-35CD-4590-BFD9-B211349EA4AC}]
        If errata workarouund is not applied  [/cover] */
        if(RegUpdated != 1U)
        {
      ...
      {
       /* Calculate the shifted duty of the channel*/
       /* MISRA2012_RULE_1_3_JUSTIFICATION: Condition of executing this case
        when period=0 will never occur, Because when period is 0 duty is set
        to 0 in GetAbsvaledge API, and if duty is 0 if(Dutycycle == 0U)
        case is executed.*/
        ShiftedDuty = (Shift + Dutycycle) % Period;
        /* [cover parentID={58B08693-F5E1-434b-932B-C5BFB2D752F4}]
        If errata workarouund is not applied  [/cover] */
        RegUpdated = Pwm_lTomCheckDuty(ChannelNumber, CoreId, Shift, \
                                       ShiftedDuty, Shift);
        /* [cover parentID={6C58737F-69C0-4ab8-A824-D231DE76C3DA}]
        If errata workarouund is not applied  [/cover] */
        if(RegUpdated != 1U)
        {
...
)
{
  ...
  {
    /* [cover parentID={8C5453B6-AA5F-4b70-A6E6-EC2A155F844E}]
       If HW Errata GTM_AI.517 and GTM_AI.522 is applicable and workaround
       applied if applicable [/cover]*/
    RegUpdated = Pwm_lCheckClkDiv(ChannelNumber, CoreId, Period, Dutycycle);
    /* [cover parentID={2B4F658F-1B30-4dd9-A9D4-AD415F7DBFE7}]
    If errata workaround is not applied [/cover] */
    if(RegUpdated != 1U)
    {
      ...
      if(Coherency == PWM_17_GTMCCU6_NON_COHERENT)
      {
        ...
        {
           /* MISRA2012_RULE_1_3_JUSTIFICATION: Condition of executing this
              case when period=0 will never occur, Because when period is 0 
              duty is set to 0 in GetAbsvaledge API, and if duty is 0
              if(Dutycycle == 0U) case is executed.*/
          ShiftedDuty = (Shift + Dutycycle) % Period;
          /* [cover parentID={5A6034A2-5860-4fa6-995D-72B3B8C43744}]
            If HW Errata GTM_AI.517 and GTM_AI.522 is applicable and workaround
            applied if applicable [/cover]*/
          RegUpdated = Pwm_lCheckClkDiv(ChannelNumber, CoreId, Shift,
                                        ShiftedDuty);
          /* [cover parentID={117AAD62-4151-429f-BE35-80A08309D126}]
          If errata workaround is not applied [/cover] */
          if(RegUpdated != 1U)
          {
      ...
      else
      {
        ...
        else
        {
          /* MISRA2012_RULE_1_3_JUSTIFICATION: Condition of executing this
              ...
              if(Dutycycle == 0U) case is executed.*/
          ShiftedDuty = (Shift + Dutycycle) % Period;
          /* [cover parentID={D1284D13-1A7A-4f95-951F-F09255056678}]
            If HW Errata GTM_AI.522 is applicable and workaround
            applied if applicable [/cover]*/
          RegUpdated = Pwm_lCheckClkDiv(ChannelNumber, CoreId, Shift,
                                        ShiftedDuty);
          /* [cover parentID={55A4AC32-B267-49c7-8845-327014DB602C}]
          If errata workaround is not applied [/cover] */
          if(RegUpdated != 1U)
          {
            ...
            Enable shadow transfer [/cover] */
            Mcu_17_Gtm_AtomChUpdateEnDis(TimerModuleNumber, \
                             TimerChannelNumber, MCU_GTM_TIMER_UPDATE_DISABLE);
            ...
            Disable shadow transfer [/cover] */
            Mcu_17_Gtm_AtomChUpdateEnDis(TimerModuleNumber,  \
                              TimerChannelNumber, MCU_GTM_TIMER_UPDATE_ENABLE);
            SchM_Exit_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
          }
        }
      }
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(Dutycycle == 0U)
      {
        /* [cover parentID={1E052527-D0D3-4622-A863-B2BFF8F59CF0}]
        If HW Errata GTM_AI.522 is applicable and workaround applied
           if applicable [/cover]*/
        RegUpdated = Pwm_lTomCheckDuty(ChannelNumber, CoreId, Period, \
                                    Dutycycle, LocalPwmChanPtr->PwmShiftValue);
        /* [cover parentID={73FC51F2-2D06-466b-B1BE-8DA11F01B8F3}]
          If errata workaround is not applied [/cover] */
        if(RegUpdated != 1U)
        {
...
)
{
  ...
  {
    /* [cover parentID={885FC4AD-3080-48c3-8E7E-7D75C04E6E81}] 
       If HW Errata GTM_AI.517 and GTM_AI.522 is applicable and workaround
       applied if applicable [/cover]*/
    RegUpdated = Pwm_lCheckClkDiv(ChannelNumber, CoreId, Period, Dutycycle);
    /* [cover parentID={2225CADC-0E61-4165-B3D0-C5B7D5CC0DCF}] 
       If errata workaround is not applied [/cover]*/
    if(RegUpdated != 1U)
    {
...
** Parameters (out): None                                                     **
**                                                                            **
** Return value:      0 - Errata not applicable and workaround is not applied **
**                    1 - Errata applicable and workaround is applied         **
**                                                                            **
** Description :     Function to check if errata GTM_AI.517 is                **
**                   applicable for ATOM Channels and workaround applied      **
**                   accordingly                                              **
**                                                                            **
** Traceability :    [cover parentID={4B5F1497-25CB-41a5-AFC1-51D842985E75}]  **
**                   [/cover]                                                 **
*******************************************************************************/
static uint8 Pwm_lCheckClkDiv(const Pwm_17_GtmCcu6_ChannelType ChannelNumber,
                              const uint8 CoreId,
                              ...
                              const uint32 Dutycycle)
{
  ...
  uint8 u8TmpSL = 0U;
  uint8 RetVal = 0U;
  uint16 ChannelInfo = 0U;
  uint8 RefChanNum = 0U;

  /* Extract channel configuration */
  PwmCorePtr = Pwm_ConfigPtr->PwmCoreAdd[CoreId];
  LocalPwmChanPtr =  &(PwmCorePtr->Pwm_ChannelConfigPtr[ChannelNumber]);
  ...
  Coherency = Pwm_lCoherency(LocalPwmChanPtr->PwmChannelInfo);
  /* Extracting the clock divider value */
  ClockDiv = LocalPwmChanPtr->ClkDiv;
  RefChanNum = (uint8)((LocalPwmChanPtr->RefChannel &\
                                  GTM_CHANNEL_NO_MASK) >> GTM_CHANNEL_NO_OFF);
  /* Extracting the RST_CCU0 conf */
  ChannelInfo = LocalPwmChanPtr->PwmChannelInfo;
  ResetType = (uint8) \
        (((uint16)(ChannelInfo) >> PWM_RSTSHFT_POS) & (PWM_RSTSHFTSTATE_MASK));
  ...
  AtomChannelRegPtr = \
                PWM_GTM_ATOM_CH_POINTER(TimerModuleNumber, TimerChannelNumber);
  /* [cover parentID={FC803AA4-721C-461d-B9F0-FFDA96FA8C19}]
   If RST_CCU0 conf is 1 and clock divider value is a valid value[/cover] */
  if((ResetType != 0U) && (RefChanNum != 0xFFU) && \
     ((ClockDiv > 1U) && (ClockDiv != 0xFFFFFFFFU)))
  {
    /* [cover parentID={7F9DE8EE-3101-4a94-A4EC-1D75A0AB9E13}]
    if duty cycle is 1 and Period is greater than 1 [/cover] */
    if((Dutycycle == 1U) && (Period > 1U))
    {
      /* [cover parentID={5A8CFFDF-DEA2-4795-ABDD-763297939C95}]
        Enter critical section [/cover] */
      SchM_Enter_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
      /* [cover parentID={D4F67AD0-4C4A-42ee-BB96-B1577D9CF555}]
      If channel is non coherent [/cover] */
      if(Coherency == PWM_17_GTMCCU6_NON_COHERENT)
      {
        /* [cover parentID={BEBFADA5-1E1A-4810-9811-3892A7B7D2DD}]
          Switch the values of the compare match registers[/cover]*/
        AtomChannelRegPtr->CM0.U = Dutycycle;
        AtomChannelRegPtr->CM1.U = Period;
        /* [cover parentID={625B5796-2B3B-4543-ADE4-EEAF4974C74F}]
        Invert the SL bit of the CTRL register [/cover]*/
        u8TmpSL = AtomChannelRegPtr->CTRL.B.SL;
        AtomChannelRegPtr->CTRL.B.SL = ((uint8)(~u8TmpSL) & PWM_CONST_1);
      }
      else
      {
        /* If channel is coherent*/
        /* [cover parentID={9024D9F8-D85D-45cb-A933-65B6EA2A5951}]
          Disable shadow transfer [/cover] */
        Mcu_17_Gtm_AtomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                      MCU_GTM_TIMER_UPDATE_DISABLE);
        /* [cover parentID={34D38475-AC84-4852-B02C-5379C62E2F6D}]
        Update shadow register with duty cycle and period [/cover] */
        AtomChannelRegPtr->SR0.U = Dutycycle;
        AtomChannelRegPtr->SR1.U = Period;
        /* [cover parentID={BA94680E-B759-4448-867E-983752E3B015}]
        Invert the SL bit of the CTRL register [/cover]*/
        u8TmpSL = AtomChannelRegPtr->CTRL.B.SL;
        AtomChannelRegPtr->CTRL.B.SL = ((uint8)(~u8TmpSL) & PWM_CONST_1);
        /* [cover parentID={C07D914B-EAFB-4a4f-A083-63A93AFD3846}]
          Enable shadow transfer [/cover] */
        Mcu_17_Gtm_AtomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                    MCU_GTM_TIMER_UPDATE_ENABLE);
      }
      /* [cover parentID={4EED29A8-6BD4-4b73-B4F7-F8930D84A79C}]
        Exit critical section [/cover] */
      SchM_Exit_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
      RetVal = 1U;
    }
    ...
    {
      /* Errata conditions not met for GTM_AI.517 for period and duty cycle*/
      /* Extracting the configured SL bit of the CTRL register */
      ValPrevSL = (uint8)AtomChannelRegPtr->CTRL.B.SL;
      /* Extracting the current SL bit of the CTRL register */
      Temp = (uint8)((PwmGtmTimerPtr->TimerChCtrlReg) >> (uint8)11U);
      Temp = Temp & (uint8)0x01U;
      /* [cover parentID={F7F3671E-01FF-4d9c-97EC-DCE2EA418173}]
      if the configured SL bit does not match the current SL bit [/cover] */
      if(ValPrevSL != Temp )
      {
        /* [cover parentID={D60272C6-326F-4c61-B39F-430B1A5E50C0}]
        Invert the SL bit of the CTRL register [/cover]*/
        u8TmpSL = AtomChannelRegPtr->CTRL.B.SL;
        AtomChannelRegPtr->CTRL.B.SL = ((uint8)(~u8TmpSL) & PWM_CONST_1);
      }
  ...
  /* [cover parentID={39F963BC-3853-4275-85CF-16F49DBBD043}]
      Check if errata GTM_AI.522 is applicable for ATOM channels
      and workaround is applied[/cover]*/
  RetVal = Pwm_lAtomCheckDuty(ChannelNumber, CoreId, Period,
                              Dutycycle, RetVal);
...
/*******************************************************************************
** static uint8 Pwm_lAtomCheckDuty                                            **
**              (                                                             **
**                const Pwm_17_GtmCcu6_ChannelType ChannelNumber,             **
**                const uint8 CoreId,                                         **
**                const uint32 Period,                                        **
...
**                   Period : Desired Period value                            **
**                   Dutycycle : Desired DutyCycle value                      **
**                   Err517Stat : Retain status of whether workaround of      **
**                                errata GTM_AI.517 is applied                **
**                                                                            **
**                                                                            **
** Parameters (out): None                                                     **
**                                                                            **
...
**                    1 - Errata applicable and workaround is applied         **
**                                                                            **
** Description :     Function to check if errata GTM_AI.522 is                **
**                   applicable for ATOM Channels and workaround applied      **
**                   accordingly                                              **
**                                                                            **
** Traceability :    [cover parentID={BD51AD48-6828-4f83-A35E-5312D17497FB}]  **
**                   [/cover]                                                 **
*******************************************************************************/
static uint8 Pwm_lAtomCheckDuty(const Pwm_17_GtmCcu6_ChannelType ChannelNumber,
                                const uint8 CoreId,
                                ...
                                const uint8 Err517Stat)
{
  ...
  const Pwm_17_GtmCcu6_CoreConfigType     *PwmCorePtr;
   /* Ptr to ATOM Channel Reg */
  Ifx_GTM_ATOM_CH* AtomChannelRegPtr;
  /* Ptr to Fixed PWM ATOM Channel Reg */
  const Ifx_GTM_ATOM_CH* FixedAtomChannelRegPtr;
  const Mcu_17_Gtm_TomAtomChConfigType *PwmGtmTimerPtr;
  uint8 ResetType;
  uint8 TimerChannelNumber;
  uint8 TimerModuleNumber;
  uint32 PrevDuty;
  uint8 Coherency;
  uint8 RetVal = Err517Stat;
  uint16 ChannelInfo = 0U;
  uint8 RefChanNum = 0U;
  uint32 FixedChanMaxVal = 0U;
  ...
  /* [cover parentID={EB9C28E9-B19A-4c76-BC0E-E71AAD0191CA}]
  Reset type is not 0 and channel is coherent and reference channel
  value is valid[/cover] */
  if((ResetType != 0U) && (Coherency != PWM_17_GTMCCU6_NON_COHERENT)&&  \
     (RefChanNum != 0xFFU))
  {
    /* Extracting the configured duty cycle */
    PrevDuty = AtomChannelRegPtr->SR1.U;
    /* [cover parentID={7C29CF80-C98F-4365-808E-5753C293701A}]
    if current duty cycle is 0% and initial duty cycle is 100%[/cover] */
    if((Dutycycle == 0U) && (PrevDuty == PWM_MAX_TIMER))
    {
      /* MISRA2012_RULE_11_3_JUSTIFICATION: SFR access. No side effects
       * foreseen by violating this MISRA rule, as the pointer to the object
       * type it is getting cast into is a known type . */
      ...
      /* [cover parentID={C8393BCB-5D48-4927-8A02-3BD3304DF8FA}]
         Extract the Period of the Fixed PWM ATOM Channel[/cover] */
      FixedAtomChannelRegPtr = \
                PWM_GTM_ATOM_CH_POINTER(TimerModuleNumber, RefChanNum);

      FixedChanMaxVal = FixedAtomChannelRegPtr->CM0.U;
      /* [cover parentID={06EB420B-4C39-4eb3-8857-9016E0BEBF61}]
         If period of the channel is MAX[/cover] */
      if(Period == FixedChanMaxVal)
      {
        /* [cover parentID={EDE835CA-877F-4a00-A786-430D2DF8E8DC}]
        Enter critical section [/cover] */
        SchM_Enter_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
        /* [cover parentID={867AB485-EFA3-467f-95BA-1077990F8A5C}]
        Disable shadow transfer [/cover] */
        Mcu_17_Gtm_AtomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                     MCU_GTM_TIMER_UPDATE_DISABLE);
        /* [cover parentID={43E96A4D-23A4-4abe-A547-ABB9F9D2896A}]
        Update CM1 with MAX ATOM timer value [/cover]*/
        AtomChannelRegPtr->CM1.U = FixedChanMaxVal;
        /* [cover parentID={887D5AC1-E672-4793-AAFC-2FCAA97CA7FE}]
        Update shadow register with duty cycle and period [/cover] */
        AtomChannelRegPtr->SR0.U = PWM_MAX_TIMER;
        AtomChannelRegPtr->SR1.U = 0x0U;
        /* [cover parentID={FB057354-81FB-4232-8F78-682617E239C3}]
        Enable shadow transfer [/cover] */
        Mcu_17_Gtm_AtomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                     MCU_GTM_TIMER_UPDATE_ENABLE);
        /* [cover parentID={10372220-560C-4061-8A74-8761D6D6BDF3}]
        Exit critical section [/cover] */
        SchM_Exit_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
        RetVal = 1U;
      }
      /* [cover parentID={3258A9CA-CB69-4429-8675-DF71A7780F60}]
         If period is more than 0[/cover] */
      else if(Period > 0U)
      {
        /* [cover parentID={1E4ED5B6-79D6-47c8-A9A8-2FEEF287DD36}]
        Enter critical section [/cover] */
        SchM_Enter_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
        /* [cover parentID={104681CA-73CA-4002-BC28-CB76C445CBD1}]
        Disable shadow transfer [/cover] */
        Mcu_17_Gtm_AtomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                     MCU_GTM_TIMER_UPDATE_DISABLE);
        /* [cover parentID={2718FF92-8524-4e73-99A3-C9FDFDBCA5A3}]
        Update CM1 register with maximum value of fixed register [/cover] */
        AtomChannelRegPtr->CM1.U = FixedChanMaxVal;
        /* [cover parentID={0F36BDE9-BF5E-4ced-9338-89E362829CA3}]
        Update shadow register with duty cycle and period [/cover] */
        AtomChannelRegPtr->SR0.U = Period;
        /* MISRA2012_RULE_1_3_JUSTIFICATION: Condition of executing this case
          when period=0 will never occur, Because when period is 0 duty is set
          ...
          case is executed.*/
        AtomChannelRegPtr->SR1.U = Dutycycle;
        /* [cover parentID={148DFA9E-7088-4bee-A3B6-1BB6049D7CE0}]
        Enable shadow transfer [/cover] */
        Mcu_17_Gtm_AtomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                     MCU_GTM_TIMER_UPDATE_ENABLE);
        /* [cover parentID={AEBE2C96-5CC2-42da-81F3-7EF8336AD866}]
        Exit critical section [/cover] */
        SchM_Exit_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
        RetVal = 1U;
      }
  ...
  return RetVal;
    ...
    (PWM_17_GTMCCU6_SET_OUTPUT_TO_IDLE_API == STD_ON))
/*******************************************************************************
** static uint8 Pwm_lTomCheckDuty                                             **
**              (                                                             **
**                const Pwm_17_GtmCcu6_ChannelType ChannelNumber,             **
**                const uint8 CoreId,                                         **
**              )                                                             **
...
**                                                                            **
** Description :     Function to check if errata GTM_AI.522 is                **
**                   applicable for TOM Channels and workaround applied       **
**                   accordingly. This local API is called when the channel   **
**                   is coherent.                                             **
**                                                                            **
** Traceability :    [cover parentID={665C4273-290C-4395-A444-3544FC3D9D5A}]  **
**                   [/cover]                                                 **
*******************************************************************************/
static uint8 Pwm_lTomCheckDuty(const Pwm_17_GtmCcu6_ChannelType ChannelNumber,
                               const uint8 CoreId,
                               ...
                               const uint32 Shift)
{
  ...
  const Pwm_17_GtmCcu6_CoreConfigType     *PwmCorePtr;
  Ifx_GTM_TOM_CH* TomChannelRegPtr; /* Ptr to TOM Channel Reg */
  const Ifx_GTM_TOM_CH* FixedTomChannelRegPtr;
  const Mcu_17_Gtm_TomAtomChConfigType *PwmGtmTimerPtr;
  uint8 TimerChannelNumber;
  uint8 TimerModuleNumber;
  uint8 RetVal = 0U;
  ...
  /* Extracting the configured duty cycle */
  PrevDuty = TomChannelRegPtr->SR1.U;
  /* [cover parentID={3B30C155-9D23-4743-ABC0-43286F2F915F}]
      If current duty cycle is 0% and initial duty cycle is 100%
      and reference channel number is a valid value[/cover] */
  if((Dutycycle == 0U) && (PrevDuty == PWM_MAX_TIMER_TOM) &&  \
     (RefChanNum != 0xFFU))
  {
    /* MISRA2012_RULE_11_3_JUSTIFICATION: SFR access. No side effects
     * foreseen by violating this MISRA rule, as the pointer to the object
    ...
    /* MISRA2012_RULE_11_5_JUSTIFICATION: SFR access. No side effects
     * foreseen by violating this MISRA rule. */
    /* [cover parentID={C9923EB3-CAF7-4c3e-B4F7-943B6A95C946}]
      Enter critical section [/cover] */
    FixedTomChannelRegPtr = \
              PWM_GTM_TOM_CH_POINTER(TimerModuleNumber, RefChanNum);

    FixedChanMaxVal = FixedTomChannelRegPtr->CM0.U;
    /* [cover parentID={D671633E-3004-4786-9369-768CA3F17216}]
      If period of the channel is MAX[/cover] */
    if(Period == FixedChanMaxVal)
    {
      /* [cover parentID={149C21FA-2589-469a-8949-F39399ABD926}]
      Enter critical section [/cover] */
      SchM_Enter_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
      /* [cover parentID={363F907D-2ED0-4a4a-8124-8F043DBF113D}]
      Disable shadow transfer [/cover] */
      Mcu_17_Gtm_TomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                   MCU_GTM_TIMER_UPDATE_DISABLE);
      /* [cover parentID={951DA939-1E1A-4daf-9434-F507E15BC48A}]
      Update CM1 with MAX ATOM timer value [/cover]*/
      TomChannelRegPtr->CM1.U = FixedChanMaxVal;
      /* [cover parentID={3E990CD5-D299-4e27-8AB6-40B40F4CE273}]
      Update shadow register with duty cycle and period [/cover] */
      TomChannelRegPtr->SR0.U = PWM_MAX_TIMER_TOM;
      TomChannelRegPtr->SR1.U = 0x0U;
      /* [cover parentID={5A148A6E-96C1-43c6-8982-77252B0EA36D}]
      Enable shadow transfer [/cover] */
      Mcu_17_Gtm_TomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                   MCU_GTM_TIMER_UPDATE_ENABLE);
      /* [cover parentID={52D8AF67-21C4-49ba-A806-A1C7287A6E61}]
      Exit critical section [/cover] */
      SchM_Exit_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
      RetVal = 1U;
    }
    /* [cover parentID={ED53C13C-06B8-44fd-BF7F-F53A34E4745A}]
         If period is more than 0[/cover] */
    else if(Period > 0U)
    {
      /* [cover parentID={5B7980B5-A8F7-43d8-8E30-4662FAC16C36}]
      Enter critical section [/cover] */
      SchM_Enter_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
      /* [cover parentID={814553D9-D688-4dc3-8A5B-4E28499192C4}]
      Disable shadow transfer [/cover] */
      Mcu_17_Gtm_TomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                   MCU_GTM_TIMER_UPDATE_DISABLE);
      /* [cover parentID={1C36C340-A966-48c3-903F-E8F9A773DB9E}]
      Update CM1 register with value of the fixed PWM channel [/cover] */
      TomChannelRegPtr->CM1.U = FixedChanMaxVal;
      /* [cover parentID={BD708FAE-C054-44e2-A6D9-158F0D697949}]
      Update shadow register with period and duty cycle [/cover] */
      TomChannelRegPtr->SR0.U = Shift;
      /* MISRA2012_RULE_1_3_JUSTIFICATION: Condition of executing this case
        when period=0 will never occur, Because when period is 0 duty is set
        ...
        case is executed.*/
      TomChannelRegPtr->SR1.U = Dutycycle;
      /* [cover parentID={385A674B-9612-445d-8D8A-7D51E437F493}]
      Enable shadow transfer [/cover] */
      Mcu_17_Gtm_TomChUpdateEnDis(TimerModuleNumber, TimerChannelNumber, \
                                   MCU_GTM_TIMER_UPDATE_ENABLE);
      /* [cover parentID={E3C8DCFF-D5C8-4fcd-A437-129B41A6A17E}]
      Exit critical section [/cover] */
      SchM_Exit_Pwm_17_GtmCcu6_PeriodAndDutyUpdate();
      RetVal = 1U;
    }
