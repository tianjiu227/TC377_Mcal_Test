...
**                                                                            **
**  TRACEABILITY : [cover parentID={36369FEE-198F-4830-8AC3-FC974DFDA4C9}]    **
**                                                                            **
  ...
  for(ModIndex = 0U; ModIndex < SPI_MAX_HW_UNIT; ModIndex++)
  {
    if(CoreConfigPtr->QSPIHwConfigPtr[ModIndex] != NULL_PTR)
    {
           ...
           CoreConfigPtr->QSPIHwConfigPtr[ModIndex]->DMATxChannel))
      {
        ...
        InvalidChannel = 0;
        #endif
        /* [cover parentID={CB02D7A7-17D6-4009-AA83-C63D141BF49D}][/cover] */
        if(Spi_RuntimeKernelVar[ModIndex]->AsyncComStatus == SPI_BUSY)
        {
        /* Call error handler */
...
static void Spi_lErrorHandler(const uint8 Module)
{
  ...
  /* [cover parentID={A10F8DCC-4F61-4d41-A4E1-3A3EFF05FD92}][/cover] */
  /* Disable DMA Rx Channel */
  Dma_ChStopTransfer(CoreConfigPtr->QSPIHwConfigPtr[Module]->DMARxChannel);
...
static void Spi_lModMainFunction(const uint8 Module)
{
      ...
      == 0U)
  {
    ...
    /* Check if PT2 status enabled and DMA RX chnnel is idle */
    if((Pt2Status == SPI_TRIGGER_ENABLE) && (((uint32)ChannelEvent & \
       (uint32)DMA_EVENT_CH_RUNNING) != (uint32)DMA_EVENT_CH_RUNNING))
    {
