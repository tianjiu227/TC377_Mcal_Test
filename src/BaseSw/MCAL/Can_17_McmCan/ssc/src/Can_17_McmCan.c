    ...
    const Can_17_McmCan_CoreConfigType *const CoreConfigPtr)
{
  ...
  if (CAN_17_MCMCAN_ADDRESS_CHECK != RxReadStartAddr)
  {
    ...
    /* [cover parentID={FEBFC079-2B6C-48b5-9C6F-9CCA365D886F}]
    CAN FD support is enabled for the controller
    [/cover] */
    #if (CAN_17_MCMCAN_FD_ENABLE == STD_ON)
    /* Convert DLC length to Number of Data Bytes supported by Hw */
    ...
    #else
    /* [cover parentID={DCE4128B-7A6A-45b8-ACD9-3091E19C9FB6}]
    Check if the DLC of received frame is greater than 8 incase of standard CAN
    [/cover] */
    if(RxMsgDLC > CAN_17_MCMCAN_MAX_DATA_LENGTH)
    {
        /* Limit the DLC length to 8 incase of standard CAN */
        RxMsgDLC = (uint8)CAN_17_MCMCAN_MAX_DATA_LENGTH;
    }
    #endif /* #if (CAN_17_MCMCAN_FD_ENABLE == STD_ON) */
