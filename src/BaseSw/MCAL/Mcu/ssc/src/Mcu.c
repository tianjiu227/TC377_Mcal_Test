...
/*CMU_CLK_CTRL write Mask*/
#define MCU_GTM_CMU_CLK_CTRL_WRITE_MASK  (0x00000100U)
...
#if (MCU_STDBY_SET_API == STD_ON)
/*Mask for PMSWCR0 VEXT STDBY EN bit*/
#define MCU_PMSWCR0_VEXTSTBYEN_MASK ((uint32)0x00000004U)
#endif
...
/* If Standby control API is available */
#if (MCU_STDBY_SET_API == STD_ON)
/*******************************************************************************
** Traceability    : [cover parentID={9D03B1BF-9792-4e6f-A8D5-82B56CC308AB}]  **
**                                                                            **
** Syntax          : void Mcu_VextStdbyCtrl                                   **
**                   (                                                        **
**                     const unit8 SetValue                                   **
**                   )                                                        **
**                                                                            **
** Description     : This API enables the user to set value for the register  **
**                   to enter standby mode when Vext supply is ramped down    **
...
** Reentrancy      : Non-Reentrant                                            **
**                                                                            **
** Parameters (in) : SetValue - Variable to enable or disable the control     **
**                                                                            **
** Parameters (out): None                                                     **
**                                                                            **
** Return value    : None                                                     **
...
*******************************************************************************/
/*[cover parentID={E9E7877F-B957-480b-9D6D-10CC6AE0AA86}][/cover]*/
void Mcu_VextStdbyCtrl(const uint8 SetValue)
{
  /* MISRA2012_RULE_19_2_JUSTIFICATION: RAM variable of SFR union type
   defined to modify certain bits and avoid 32-bit mask value.
   No side effects foreseen by violating this MISRA rule.*/
  Ifx_PMS_PMSWCR0 Pmswcr0Val;

  /*[cover parentID={764C7A20-F934-497b-82BC-A4D0BE09DAF5}][/cover]
  Is the Mcu driver initialized?*/
  if(Mcu_DriverState != MCU_DRIVER_INITIALIZED)
  {
    /*[cover parentID={51288487-1B0D-401a-BAD2-F0CE63460F95}][/cover]*/
    /*If DET or Safety is ON*/
    #if ((MCU_DEV_ERROR_DETECT == STD_ON) || (MCU_SAFETY_ENABLE == STD_ON))
    /*[cover parentID={71635667-1D75-4dcb-93BC-231678899C5C}][/cover]*/
    /*Report Det or Safety error*/
    Mcu_lReportError(MCU_SID_VEXTSTDBYCTRL, MCU_E_UNINIT);
    #endif
  }
  else
  {
    /*[cover parentID={F4408933-B00B-4f87-95B8-3A8839870CC3}][/cover]
    Is SetValue input 0x01?*/
    if((SetValue == 0x01U)||(SetValue == 0x00U))
    {
      /*Get current PMS_PMSWCR0 register value*/
      Pmswcr0Val.U = PMS_PMSWCR0.U;
      Pmswcr0Val.U &= ~MCU_PMSWCR0_VEXTSTBYEN_MASK;
      /*Calculate the PMS_PMSWCR0 register value with SetValue*/
      Pmswcr0Val.U |= (MCU_PMSWCR0_VEXTSTBYEN_MASK & 
                      ((uint32)SetValue << (uint32)2));
      /*Enable the Standby mode entry on Vext supply ramp down*/
      MCU_LIB_RUNTIME_WRITESAFETYENDINITPROTREG(&PMS_PMSWCR0.U, 
      Pmswcr0Val.U);
    }
    else
    {
      /*[cover parentID={96E9BAC3-C568-4d74-AC4F-1995B2F3CC7B}][/cover]*/
      /*If DET or Safety is ON*/
      #if ((MCU_DEV_ERROR_DETECT == STD_ON) || (MCU_SAFETY_ENABLE == STD_ON))
      /*[cover parentID={085D58F6-FD46-44fb-8E27-86B8434EFAEA}][/cover]*/
      /*Report Det or Safety error*/
      Mcu_lReportError(MCU_SID_VEXTSTDBYCTRL, MCU_E_PARAM_STDBY);
      #endif
    }
  }
}
#endif
/* End of #if (MCU_STDBY_SET_API == STD_ON) */
...
LOCAL_INLINE void Mcu_lGtmCmuGlobalInit(void)
{
  ...
  /* MISRA2012_RULE_19_2_JUSTIFICATION: RAM variable of SFR union type
     defined to modify certain bits and avoid 32-bit mask value.
     No side effects foreseen by violating this MISRA rule.*/
  Ifx_GTM_CMU_CLK_CTRL GtmCmuClkCtrlval;
  ...
  */
  ...
  /*Configure CMU_CLK_CTRL register for Clock source selection*/
  GtmCmuClkCtrlval.U = GTM_CMU_CLK_CTRL.U;
  GtmCmuClkCtrlval.U &= ~MCU_GTM_CMU_CLK_CTRL_WRITE_MASK;
  GtmCmuClkCtrlval.U |= MCU_GTM_CMU_CLK_CTRL_WRITE_MASK & 
           (Mcu_ConfigPtr->McuGtmConfigPtr->GtmClockCfgPtr->GtmCmuClkCtrl<<8U);
  GTM_CMU_CLK_CTRL.U = GtmCmuClkCtrlval.U;
...
LOCAL_INLINE void Mcu_lGtmCmuGlobalDeInit(void)
{
  ...
  /*Write CMU clock source selection register*/
  GTM_CMU_CLK_CTRL.U = MCU_GTM_CMU_CLK_CTRL_RESET_VAL;
  /*
   * Numerator is written twice before writing into Denominator
   * This is secure way of updating these registers as per TS.
   */
...
LOCAL_INLINE Std_ReturnType Mcu_lGtmCmuGlobalInitCheck(void)
{
  ...
  SfrVal    = GTM_CMU_CLK_CTRL.U & MCU_GTM_CMU_CLK_CTRL_WRITE_MASK;
  ConfigVal = MCU_GTM_CMU_CLK_CTRL_WRITE_MASK &
           (Mcu_ConfigPtr->McuGtmConfigPtr->GtmClockCfgPtr->GtmCmuClkCtrl<<8U);
  CmpVal   &= ~(SfrVal ^ ConfigVal);
